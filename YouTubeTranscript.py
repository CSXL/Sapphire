...
def get_transcript(video_id: str) -> str:
    ...
    transcript = modify_transcript(200, 2000, video_id, string_format)  # Assuming this is defined elsewhere
    return transcript
...
def modify_transcript(limit1: str, limit2: str, video_id: str, string_format: str) -> str:
    """
    ...
    """
    if get_youtube_video_duration(video_id, os.getenv('api_key')) >= float(limit1) \
...
def join_most_sophisticated_sentences(text, num_sentences):
    ...
    sorted_sentences = sorted(sentence_scores.items(), key=lambda x: \
                          x[1]['compound'], reverse=True)
    # Extract the most sophisticated sentences
    sophisticated_sentences = [sentence for (sentence, _) in
                           sorted_sentences[:num_sentences]]
